...
*   **Processing:** Recursively traverses the `constituents_tree`. For each node labeled "NP":
    ...
    4.  If token indices are found, creates a spaCy `Span` using `_get_span_from_token_indices(sent, start_idx, end_idx)`. This mapping links Benepar's grammatical structure (the Tree node) to spaCy's token-based representation (the Span). This mapping can fail if Benepar's and spaCy's tokenization differ significantly for that phrase (logs a warning).
